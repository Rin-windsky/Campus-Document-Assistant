      ...
      <aside class="ref-panel">
        <div v-if="selectedCitation">
          ...
          <div class="ref-detail-card">
            ...
            <button class="ref-goto-btn" @click="gotoDoc(selectedCitation.doc)">
...
function gotoDoc(docName) {
  router.push({ path: '/docs', query: { doc: docName } })
}
...
/* AI 回答：无气泡，纯文本，居中 */
.msg-wrap.ai .msg-body {
  font-size: 0.9375rem;
  line-height: 1.8;
  color: var(--text);
}

/* 用户消息：气泡右对齐，与 AI 文本右边缘一致 */
...
.ref-goto-btn:hover { background: var(--primary); color: #fff; }
